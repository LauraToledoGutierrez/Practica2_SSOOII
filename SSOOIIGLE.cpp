...
Request cogerPeticion(){
    int random=1+ rand()% (100 - 1);
    Request req;

    if(random<=80 & !clientRequestPremium.isEmpty()){
        req=clientRequestPremium.pop();
    }ifelse(ramdon>80 | clientRequestPremium.isEmpty()){
        req=clientRequestFree.pop();
    }else{
...
void findWord(int iteration, std::vector<std::string> vector)
{
    ...
    std::string palabra = peticion.getwordToSearch();
    peticion=cogerPeticion();
    ...
    for (int i = 0; i < vector.size(); i++)
    {
        ...
        for (int j = 0; j < words.size(); j++)
        {
            ...
            if (words[j].compare(palabra) == 0)
            {
                resultLine = threadFinder[iteration].getInitialLine(); //NOSE
